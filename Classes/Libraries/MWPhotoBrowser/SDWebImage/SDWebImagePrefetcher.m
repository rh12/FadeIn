...
- (void)reportStatus
{
    ...
    NSLog(@"Finished prefetching (%d successful, %d skipped, timeElasped %.2f)", (uint)(total - _skippedCount), (uint)_skippedCount, CFAbsoluteTimeGetCurrent() - _startedTime);
}
...
{
    _finishedCount++;
    NSLog(@"Prefetched %d out of %d", (uint)_finishedCount, (uint)[self.prefetchURLs count]);
...
{
    _finishedCount++;
    NSLog(@"Prefetched %d out of %d (Failed)", (uint)_finishedCount, (uint)[self.prefetchURLs count]);
