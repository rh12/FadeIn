...
@property (nonatomic, retain) UIAlertController *actionsSheet;
...
//        self.wantsFullScreenLayout = YES;
        ...
        // Customization
        shouldUpdateTitle = YES;
        shouldAlwaysUseToolbar = NO;
        shouldAutoHideControls = YES;
    }
    return self;
}
...
- (void)performLayout {
    ...
    // Toolbar
    if ([self numberOfPhotos] > 1 || _displayActionButton || shouldAlwaysUseToolbar) {
...
- (void)updateNavigation {
    ...
    if (shouldUpdateTitle) {
        if (numberOfPhotos > 1) {
            self.title = [NSString stringWithFormat:@"%i %@ %i", (uint)_currentPageIndex+1, NSLocalizedString(@"of", @"Used in the context: 'Showing 1 of 3 items'"), (uint)numberOfPhotos];
        } else {
            self.title = nil;
        }
    }
	...
	_nextButton.enabled = (numberOfPhotos > 0 && _currentPageIndex < numberOfPhotos-1);
}
...
- (void)hideControlsAfterDelay {
	if (shouldAutoHideControls && ![self areControlsHidden]) {
        [self cancelControlHiding];
...
- (void)actionButtonPressed:(id)sender {
    if (_actionsSheet) {
        ...
        if ([self numberOfPhotos] > 0 && [photo underlyingImage]) {
            ...
            // Sheet
            self.actionsSheet = [UIAlertController alertControllerWithTitle: nil
                                                                    message: nil
                                                             preferredStyle: UIAlertControllerStyleActionSheet];
            [_actionsSheet addAction:
             [UIAlertAction actionWithTitle: NSLocalizedString(@"Cancel", nil)
                                      style: UIAlertActionStyleCancel
                                    handler: ^(UIAlertAction* action) { [self hideControlsAfterDelay]; }]]; // Continue as normal...
            [_actionsSheet addAction:
             [UIAlertAction actionWithTitle: NSLocalizedString(@"Save", nil)
                                      style: UIAlertActionStyleDefault
                                    handler: ^(UIAlertAction* action) { [self savePhoto]; }]];
            [_actionsSheet addAction:
             [UIAlertAction actionWithTitle: NSLocalizedString(@"Copy", nil)
                                      style: UIAlertActionStyleDefault
                                    handler: ^(UIAlertAction* action) { [self copyPhoto]; }]];
            if ([MFMailComposeViewController canSendMail]) {
                [_actionsSheet addAction:
                 [UIAlertAction actionWithTitle: NSLocalizedString(@"Email", nil)
                                          style: UIAlertActionStyleDefault
                                        handler: ^(UIAlertAction* action) { [self emailPhoto]; }]];
            }
...
- (void)mailComposeController:(MFMailComposeViewController*)controller didFinishWithResult:(MFMailComposeResult)result error:(NSError*)error {
    if (result == MFMailComposeResultFailed) {
        UIAlertController* alert = [UIAlertController alertControllerWithTitle: @"Email failed to send. Please try again."
                                                                       message: nil
