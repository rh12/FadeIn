                  ...
                  GLfloat *objx, GLfloat *objy, GLfloat *objz)
{
    ...
    //and store in A[16]
    multMatrices4f(A, projection, modelview);
    
    //Now compute the inverse of matrix A
    if(invertMatrix4f(A, m)==0)
        return 0;
    
    //Transformation of normalized coordinates between -1 and 1
    in[0] = (winx-(GLfloat)viewport[0])/(GLfloat)viewport[2]*2.0-1.0;
    in[1] = (winy-(GLfloat)viewport[1])/(GLfloat)viewport[3]*2.0-1.0;
    in[2] = 2.0*winz-1.0;
    in[3] = 1.0;
    
    //Objects coordinates
    multVectorMatrix4f(out, m, in);
    if(out[3]==0.0)
        return 0;
    out[3] = 1.0/out[3];
    *objx = out[0]*out[3];
